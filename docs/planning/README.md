...
- [Roadmap and Stages](03-roadmap-and-stages.md): milestones, Stage 3 export, Stage 4 TCX, Stage 5 KML conversion, first tasks, and definitions of done.
...
OpenTrack Viewer is a privacy-first, 100% client-side web/PWA for viewing activity files. The app has a homepage for project description, privacy-safe SEO metadata, a viewer/process page for local activity-file processing, a routed Terms and Conditions page, and a settings modal that does not navigate away from loaded data. The homepage should also surface the dedicated GoPro/video telemetry page once available, with clear no-upload copy and routing to that separate tool. The header uses the brand/title as the home link, avoids duplicate Home navigation, omits global subtitle text, places `Tools` beside the title with `File viewer` linking to the viewer/process page, and always opens Settings from an icon. The loaded viewer uses max-width content, a large-screen left section sidebar, an overview box first, a separate device card when metadata exists, a map box second, laps beside the map when available on large screens, and charts later. The overview defaults to average pace for running and average speed for cycling, while a non-destructive `View as` selector lets any loaded activity use another presentation type. Chart capability comes from normalized data rather than sport: sport recommends the initial set, and users can select or deselect capable charts, including pace on cycling and speed on running. Device metadata stays out of the summary card. GoPro/video activities without source speed derive speed from GPS points when possible. Settings include app-wide theme modes: system, dark, and light. Charts need readable axis spacing, with distance tick marks every 1 km and time tick marks every 5 minutes where space permits, and incapable or deselected charts are omitted instead of shown as no-data or wrong-sport messages. Running cadence should be modeled and displayed as strides per minute, not RPM, and remains distinct from pedal cadence. GPX is first, FIT follows, Stage 3 adds browser-side GPX/FIT export and direct conversion between supported formats, Stage 4 adds TCX import/export, and Stage 5 adds KML import plus conversion to every compatible registered exporter with explicit loss warnings. A later GoPro video telemetry milestone adds a dedicated video telemetry extraction page under `Tools`, a post-extraction overlay milestone adds standalone activity overlays plus synchronized telemetry video overlay preview/export, and a later Route Builder milestone adds local planned-track creation, route editing, track appending, and export for external devices. Standalone activity overlays start with transparent route-map image export from GPX or viewer-handoff data. Overlay-only export should ship before burned-in browser video export unless the feasibility spike proves the heavier video render path is acceptable. Activity files, supported video telemetry, overlay previews, route drafts, and generated exports are processed in the browser without backend processing or upload.
