...
OpenTrack Viewer is a privacy-first, 100% client-side web/PWA for viewing activity files. The app has a homepage for project description, privacy-safe SEO metadata, a viewer/process page for local activity-file processing, a routed Terms and Conditions page, and a settings modal that does not navigate away from loaded data. The header uses the brand/title as the home link, avoids duplicate Home navigation, omits global subtitle text, places `Tools` beside the title with `File viewer` linking to the viewer/process page, and always opens Settings from an icon. The loaded viewer uses max-width content, a large-screen left section sidebar, an overview box first, a map box second, laps beside the map when available on large screens, and charts later. The overview uses average pace for running activities and average speed for cycling activities. Settings include app-wide theme modes: system, dark, and light. Charts need readable axis spacing, with distance tick marks every 1 km and time tick marks every 5 minutes where space permits. Running cadence should be modeled and displayed as strides per minute, not RPM. GPX is first, FIT follows, Stage 3 adds browser-side GPX/FIT export and direct conversion between supported formats, Stage 4 adds TCX import/export, a later GoPro video telemetry milestone adds a dedicated video telemetry extraction page under `Tools`, and a post-extraction overlay milestone adds synchronized telemetry overlay preview/export. Overlay-only export should ship before burned-in browser video export unless the feasibility spike proves the heavier video render path is acceptable. Activity files, supported video telemetry, overlay previews, and generated exports are processed in the browser without backend processing or upload.
