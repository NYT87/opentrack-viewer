...
| Summary while focused | Always describes the whole activity | This diverges from `AV-605`, which asks for focused-range stats in the summary. The summary panel is the *activity* summary; making its numbers change with a chart selection would make "Distance" ambiguous at a glance. Focused figures, if wanted, belong in their own labelled panel rather than by mutating this one. |
