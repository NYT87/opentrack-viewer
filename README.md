...
| Lap highlighting | Recolours the lap's stretch of route; never moves the map | §17 asked whether a lap should focus the view. It does not, deliberately: a lap says *which part of the ride this is*, not *take me there*. That is the whole difference from a chart range selection, which does move the camera, and a unit test asserts no `fitBounds` follows a lap press. |
